...
const Productsmainpage2: React.FC<PopulardishesProps> = ({ data, isLoading }) => {
  const dispatch = useDispatch();
  const [productsPerPage, setProductsPerPage] = useState<number>(
    window.innerWidth >= 1300 ? 5 : window.innerWidth < 640 ? 2 : 4
  );
    ...
    <section className="mx-auto my-10 flex w-full flex-col items-center justify-center">
      <div className="mx-auto w-full rounded-2xl">
        <div className="flex justify-end">
          <h1 className="mr-8 mb-4 border-b border-[#417F56] py-2 text-right text-2xl font-semibold text-[#313231]">
            غذای غیر ایرانی
          </h1>
        </div>
                      ...
                      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-md transition-transform duration-300 hover:scale-105">
                        ...
                        <div className="flex flex-col justify-between gap-3 p-4">
                          <div className="flex justify-between">
                            <div className="border-b border-[#417F56] text-right text-sm font-semibold">
                              {product.price} $
                            </div>
